...
\begin{proof} Theorem~\ref{thm:exceptionals}. Let $M = 22$. Since there is a full-content tableau of height $22$ and period $66$, $\pro^{22}$ has order $66$ on $\lambda_F$. Now if we evaluate $f_{\lambda_F}$ on a $66$th root of unity, we don't even get an integer (?). 
\end{proof}
...
\end{equation}
\-\ \\ \-\ 
\begin{proof} (Theorem~\ref{thm:propeller}) Fix $k$, $q$, and $d$ dividing $q$. We have that \[ f_k(\zeta) = \frac{[q]!_q [q-(k-1)]_q}{[2k-1]!_q [q-(2k-1)]!_q [k]_q }.\]  If $d = 1$, then by Equation~\ref{eq:mainresulteq}, 
