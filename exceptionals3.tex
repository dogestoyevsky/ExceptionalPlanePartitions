...
\subsection{Computation of period}\label{sec:period} We use the results of the previous section to relate the period of $T \in \inc^q(\lambda)$ under $\pro^q$ to data about $\deflate^q(T)$. Let 
\[\Sigma^q: \lbrace 0,1\rbrace^q \rightarrow \lbrace 0,1\rbrace^q\]
 be the cyclic rotation defined by 
 \[
 \Sigma^q(v_1,...,v_q) = (v_2,...,v_q,v_1)
 \]
  for $(v_1,...,v_q) \in \lbrace 0,1 \rbrace^q$. The above shows that if we define $K^q: \coprod_{s \leq q} \incgl^q(\lambda) \times \lbrace 0,1\rbrace^q \rightarrow \coprod_{s \leq q} \incgl^q(\lambda) \times \lbrace 0,1\rbrace^q$:
