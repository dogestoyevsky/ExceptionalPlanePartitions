...
\]
 is injective, $(\pro^q)^{\circ n}(T) = T$ exactly when $\compress^q \circ (\pro^q)^{\circ n}(T) = \compress^q ( T)$. So we are reduced to looking at what powers $n$ of $\defpro^q$ stabilize $\compress^q (T)$. 
Clearly we must have $\shift^n(\content(T)) = \content(T)$.  In addition, we must have that $\pro^{n'}(\deflate(T))$, where $n'$ is $n$ minus the number of times that $K^\ell(W(T))$ does not have $1$ in its content vector for $\ell \in 1,...,n$. 
