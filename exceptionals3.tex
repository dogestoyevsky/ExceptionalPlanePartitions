...
The subset $Q \subseteq \lambda \times \mathbf{m}$ is an order ideal if and only if both
\begin{itemize}
...
By assumption, $(i',j') \leq (i,j)$. Hence, by condition $(b)$, $\mathrm{ht}((i,j),Q) \leq \mathrm{ht}((i',j'),Q)$. Now, condition $(a)$ implies that \[ \mathrm{ht}((i'',j''),Q) = 1 + \max(\{ k'' \in \mathbb{Z}_{\geq 0} : (i'',j'',k'') \in Q \}). \]  Therefore there is a $k'' \geq k$ with $(i',j',k'') \in Q$. But since $k' \leq k \leq k''$, condition $(a)$ then implies $(i',j',k') \in Q$. 
\end{proof}

\begin{proposition}\label{prop:equivariant}
Let $\lambda$ be a justified convex shape. Then $\Omega$ intertwines $\mot_{\text{id},(1,1,-1)}$ and $\pro$. That is, for $P \in J(\lambda \times [m])$, we have $\Omega \circ \mot_{\text{id},(1,1,-1)}(P) = \pro^{\rank(\lambda)+m+1} \circ \Omega(P)$.
\end{proposition}
\begin{proof}
Let $a = a_1(\lambda)$ and let $b = a_2(\lambda)$, that is, $(a,b) = \aaa_{\lambda}$. Since $\lambda$ is justified, $a+b = \rank(\lambda)$.  Fix $P \in J(\lambda \times [m])$. 

Recall that for $T \in \inc^{a+b+m+1}(\lambda)$, we have by definition that
\begin{equation*}
\pro^{a+b+m+1}(T) = \rho_{a+b+m} \circ  ... \circ \rho_1(T).
\end{equation*}
...
Fix an index $\ell$ and an entry $\y = (i,j) \in \lambda$. Let $Y \coloneqq P \cap ( \y \times [m] )$ be the column of $\y$ in $P$. We consider the effect on $Y$ of applying $\tau^{a+b-(\ell-1)}_{\text{id},(1,1,-1)}$ to $P$  and the effect on the box $\aaa_\lambda - \y$ of applying $\rho_\ell$ to the increasing tableau $\Omega(P)$. Note that $t_{(i,j,i+j-(a+b-(\ell-1)))}$ is the unique toggle in the definition of $\tau^{a+b-(\ell-1)}_{\text{id},(1,1,-1)}$ (Equation~\ref{eq:taudef}) that acts on an element of $Y$.
...
On the tableaux side, the case assumption implies that $[\Omega(P)](\aaa_\lambda - \y) = \ell+1$. The $\aaa_\lambda - \y$ entry of $\Omega(P)$ is decreased to $\ell$ by $\rho_\ell$ exactly when, for all $d$ with $\aaa_\lambda - \y -\e_d \in \lambda^\star$, $[\Omega(P)](\aaa_\lambda - \y -\e_d) \leq \ell-1$. This is equivalent to the stated condition on when $\tau^{a+b-(\ell-1)}_{\text{id},(1,1,-1)}$ acts nontrivially on $Y$.
...
On the tableaux side, this case assumption implies that $[\Omega(P)](\aaa_\lambda - \y) = \ell$. The $\aaa_\lambda - \y$ entry of $\Omega(P)$ is increased to $\ell+1$ by $\rho_\ell$ exactly when, for all $d$ with $\aaa_\lambda - \y +\e_d \in \lambda^\star$, $[\Omega(P)](\aaa_\lambda - \y + \e_d)) \geq \ell+2$. This is equivalent to the stated condition on when $\tau^{a+b-(\ell-1)}_{\text{id},(1,1,-1)}$ acts nontrivially on $Y$.
...
In this final section, we collect the above results into a proof of Theorems~\ref{thm:exceptionals} and~\ref{thm:propeller}. Our strategy is as follows. By Proposition~\ref{prop:equivariant}, we can determine the orbit structure of plane partitions under $\Psi$ by studying increasing tableaux under $\pro$. By Theorem~\ref{thm:compressbijective}, increasing tableaux are characterized by the data of a gapless increasing tableau and a content vector. But $\incgl^k(\lambda)$ is a finite set for any fixed $\lambda$, so we can determine all its elements and then check their periods. These data are given in Table~\ref{tab:E6} for the $\lambda$ the Cayley-Moufang plane and Table \ref{tab:E7} for the $\lambda$ the Freudenthal poset. OLIVER ELABORATE ON HOW? In addition, the number of elements of $\binom{[M]}{k}$ whose periods divide a given divisor of $M$ are predicted by NAME THIS THING (CITE THIS FACT). Therefore we can fully characterize increasing tableaux of a given height under $\pro$.
...
$q$ & orbit size & number of orbits\\
  ...
  \hline
\end{tabular}
\caption{The number of orbits of gapless increasing tableaux in $\incgl^q(P)$ for each $q$ for the $E_6$ poset.}
\label{tab:E6}
\end{table}
...
$q$ & orbit size & number of orbits\\
 ...
 27 & 3 & 2\\ \cline{2-3}
  ...
  \hline
\end{tabular}
\caption{The number of orbits of gapless increasing tableaux in $\incgl^q(P)$ for each $q$ for the $E_7$ poset.}
\label{tab:E7}
\end{table}
...
\textsf{Input}:  $\lambda$ justified and convex shape, $L \in \mathbb{Z}_{>0}$ and the finite set $\coprod_{k} \incgl^k(\lambda)$ of all gapless tableaux associated to $\lambda$ and their periods under the corresponding $\pro^k$. \\ \noindent
\textsf{Output}: $x$, the number of $T \in \inc^q(\lambda)$ whose period divides $L$. \\  \-\ \\ 
\textsf{Initialization}: Set $x = 0$. \\
\textsf{Procedure}: Loop over gapless tableaux.
\begin{arrowlist}
\item For each gapless tableaux $T$, loop over divisors $d$ of $L$.
\begin{enumerate}
\item   Compute the number $i$ of content vectors with period exactly $L/d$. \item Compute the period associated with $T$ and a content vector of period $L/d$, as in Theorem~\ref{thm:periodthm}. 
\item If this period divides $L$, add $i$ to $x$.
\end{enumerate}
\end{arrowlist} \-\ 

\begin{theorem}~\label{thm:alg1}
The output of Algorithm 1 is equal to the number of order ideals in $J(\lambda \times [m])$ whose order under $\Omega$ divides $L$. 
\end{theorem}
\begin{proof}
By Corollary~\ref{cor:multisets}, it suffices to show that Algorithm 1 outputs the number of elements of $\inc^M(\lambda)$ whose period under $\pro^M$ divides $L$, where $M = \rank(\lambda)+m+1$. By Theorem~\ref{thm:compressbijective}, an element $T \in \inc^M(\lambda)$ corresponds to a unique pair $(S,v) \in \coprod_{k \leq M} (\incgl^k(\lambda^{\ast}) \times \binom{[M]}{k})$. Therefore, we we may instead count pairs $(S,v)$ such that $\pi(S,v)$ divides $L$. We do this by considering all such pairs: we fix a tableaux and then MORE HERE. For each pair we compute the corresponding period and add $1$ to the output variable $x$ is this period divides $L$.
\end{proof}
