...
In this final section, we collect the above results into a proof of Theorem~\ref{thm:exceptionals}. Our strategy is as follows. By Proposition~\ref{prop:equivariant}, we can determine the orbit structure of plane partitions under $\Omega$ by studying increasing tableaux under $\pro$. By Theorem~\ref{thm:compressbijective}, increasing tableaux are characterized by the data of a full content tableau and a content vector. But $\incgl^k(\lambda)$ is a finite set for any fixed $\lambda$, so we can determine all its elements and then check their periods. These data are given in Table~\ref{tab:E6} for the $\lambda$ the Cayley-Moufang plane and Table \ref{tab:E7} for the $\lambda$ the Freudenthal poset. OLIVER ELABORATE ON HOW? In addition, the number of elements of $\binom{[M]}{k}$ whose periods divide a given divisor of $M$ are predicted by NAME THIS THING (CITE THIS FACT). Therefore we can fully characterize increasing tableaux of a given height under $\pro$.
...
\textsf{Input}:  $\lambda$ justified and convex shape, $L \in \mathbb{Z}_{>0}$ and the set $\coprod_{k} \incgl^k(\lambda)$ of gapless tableaux associated to $\lambda$ and their periods under the corresponding $\pro^k$. \\ \noindent
