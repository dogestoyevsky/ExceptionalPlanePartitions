...
\begin{proof}  We proceed by induction on $k$. The base case $k=1$ is trivial for any tableau $T \in \inc^q(\lambda)$  with $1 \in \mathrm{range}(T)$, since $\lambda \left( \deflate^q(T)^{(1)} \right)$ and $\lambda \left( T^{(1)} \right)$ are then both the empty order ideal.
