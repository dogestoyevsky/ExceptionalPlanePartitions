...
First consider $\x \in \lambda$ with $\x \not \in \lambda(T^{(q)})$. Then,  $T^{(q)}(\x) = \bullet$ and so 
\[
...
Now consider $\x \in \lambda$ with $\x \in \lambda(T^{(q)})$. We observe that since $\swap_\ell$ does not affect $\swap_{N_T} \circ \cdots \circ \swap_1 \circ \rep_{1 \rightarrow \bullet} (T)$ for $\ell > i_{N_T}$, we have that
