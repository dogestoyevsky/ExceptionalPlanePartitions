...
\newcommand{\defpro}{\ensuremath{\mathfrak{pro}_{*}}}
\newcommand{\shift}{\ensuremath{\mathsf{sh}}}
...
for $1 \leq k \leq N_T$.
\end{lemma}
\begin{proof} We proceed by induction on $k$. Fix $T \in \inc^q(\lambda)$  with $1 \in \mathrm{range}(T)$. In the base case $k = 1$,  $T^{(i_1)} = T^{(1)} = \rep_{1 \rightarrow \bullet}(T)$ has $\bullet$ in exactly those boxes $\x \in \lambda$ such that $T(\x) = 1$. On the other hand, $\deflate^q(T)^{(1)} =  \rep_{1 \rightarrow \bullet}(\deflate^q(T))$ has $\bullet$ in exactly those boxes $\x \in \lambda$ such that $\deflate^q(T)(\x) = 1$. But since $1 \in \mathrm{range}(T)$, $\deflate^q(T)(\x) = 1$ if and only if $T(\x) = 1$. 


Now, say the lemma holds for all tableaux $T$ for $k < t$. First consider the case where $T^{(i_t)}(\x) = \bullet$ and $T^{(i_{t-1})}(\x) \neq \bullet$. Then, $T^{(i_{t-1})}(\x) = i_t$ and $T^{(i_{t-1})}(\x') = \bullet$ for some $\x'$ adjacent to $\x$. But since $\rep_{1 \rightarrow \bullet}$ and $\swap_j$ for  $j \leq i_{t-1}$ only act on elements of $T$ labeled with values less than or equal to $i_{t-1}$, having $T^{(i_{t-1})}(\x) = i_t$ is equivalent to having $T^{(i_{t-1})}(\x) = T(\x)$. This is, in turn, equivalent to $\deflate^q(T)(\x) = t$ and to $\deflate^q(T)^{(t-1)}(\x) = t$. Additionally, by the inductive hypothesis, $T^{(i_{t-1})}(\x') = \bullet$ if and only if $\deflate^q(T)^{(t-1)}(\x') = \bullet$. Therefore, the conditions of this case are equivalent to requiring $\deflate^q(T)^{(t)}(\x) = \bullet$ and $\deflate^q(T)^{(t-1)}(\x) \neq \bullet$. 

Consider the other possible case, where $T^{(i_t)}(\x) = \bullet$ and $T^{(i_{t-1})}(\x) = \bullet$. Here, there is no $\x'$ adjacent to $\x$ such that $T^{(i_{t-1})}(\x') = i_t$. By similar reasoning as in the previous case, these conditions are equivalent to the statement that there is no $\x'$ adjacent to $\x$ with $T(\x') = i_t$. This statement is, in turn, equivalent to saying that there is no $\x'$ adjacent to $\x$ with $\deflate^q(T)(\x') = \deflate^q(T)^{(t-1)}(\x') = t$. Since $\deflate^q(T)^{(t-1)}(\x) = \bullet$ by the inductive hypothesis, the conditions of this case are finally equivalent to requiring that $\deflate^q(T)^{(t)}(\x) = \bullet$ and $\deflate^q(T)^{(t-1)}(\x) = \bullet$.
\end{proof}
...
Now, assume that the lemma holds for all increasing tableaux $T$ in the case $k < t$. We first analyze the left side of Equation~(\ref{eq:gappy_promotion}). If $\x \in \lambda\left( \deflate^q(T)^{(t-1)} \right)$, then 
...
Now, we analyze the right side of Equation~(\ref{eq:gappy_promotion}). If $\x \in \lambda \left(T^{(i_{t-1})} \right)$, then $T^{(i_{t-1})}(\x) \leq i_{t-1}$  and so we have
\[
\left[ \swap_{\ell+1} \left(T^{(\ell)} \right) \right](\x) = T^{(\ell)}(\x)
\]
 for all $i_{t-1} \leq \ell$. Therefore, by the previous paragraph and the inductive hypothesis, this shows that Equation~(\ref{eq:gappy_promotion}) holds at all such $\x$. If $\x \in \lambda \left(T^{(i_{t})} \right) \setminus \lambda \left(T^{(i_{t-1})} \right)$, then $i_{t-1} < T_{i_{t}}(\x) \leq i_t$. Thus, we must have that $T_{i_{t}}(\x) = i_{t}$, since swaps do not change the content of a tableau. Therefore, either $T^{(i_{t-1})}(\x) = \bullet$ or $T^{(i_{t-1})}(\x) =  T(\x) = i_{t}$. In the former case, there must be a box $\x'$ adjacent to $\x$ such that $T^{(i_{t-1})}(\x') = T(\x') = i_{t}$. This occurs exactly when $\deflate^q(T)^{(t-1)}(\x') = \deflate^q(T)(\x') = t$. In the latter case, there is no $\x'$ adjacent to $\x$ such that $T^{(i_{t-1})}(\x') = \bullet$. Since we have assumed that  $1 \in \mathrm{range}(T)$, Lemma~\ref{lem:bullet_placement} applies, implying that this occurs exactly when  $\deflate^q(T)^{(t-1)}(\x') \neq \bullet$ for every box $\x'$ adjacent to $\x$. This proves that Equation~(\ref{eq:gappy_promotion}) holds for the remaining boxes.

This shows that $\lambda \left(T^{(i_k)} \right) = \lambda \left(\deflate^q(T)^{(k)} \right)$ for all $k$, and that moreover, if $h = \deflate^q(T)_{k}(\x)$, then $T_k(\x) = \inflate^q_{\content^q(T)}(h)$. 
\end{proof}
...
Let $T \in \inc^q(\lambda)$ with $1 \in \mathrm{range}(T)$. Let $1 = i_1 < \dots < i_{N_T}$ be the ordered list of indices such that $\content^q(T)(i_\ell) = 1$. Then
...
%\begin{align*}
%\deflate^q \circ &\decr \circ \rep_{\bullet \rightarrow q+1} \circ \swap_q \circ \cdots \circ \swap_2\circ \rep_{1 \rightarrow \bullet} (T) = \\
% & \decr \circ \rep_{\bullet \rightarrow N_T+1} \circ \swap_{N_T} \circ \cdots \circ \swap_2 \circ \rep_{1 \rightarrow \bullet} \circ \deflate^q (T). 
%\end{align*}

First, consider $\x \in \lambda$ with $\x \notin \lambda \left(T^{(q)} \right)$. Then,  $T^{(q)}(\x) = \bullet$ and so 
\[
\pro^q(T)(\x) =  [\decr \circ \rep_{\bullet \rightarrow q+1} (T^{(q)})](\x) = q.
\]
Now by \cite[Lemma~2.1]{DPS}, the content vector of  
$
\pro^q(T)
$
 is a cyclic rotation of the content vector of $T$. This implies that both content vectors contain the same number of $1$s. Since $q$ is the greatest element of $\mathrm{range}(\pro^q(T))$, we then have
 \[
 \deflate^q \circ \pro^q (T)(\x) = N_T.
 \]
 
  By Lemma~\ref{lem:bullet_placement}, we observe on the other hand that $\x \not \in \lambda \left( \deflate^q(T)^{(N_T)} \right)$. Hence, $\deflate^q(T)^{(N_T)}(\x) = \bullet$, and thus,
  \[
  [\pro^{N_T} \circ \deflate^q(T)](\x) = \Big[ \decr \circ \rep_{\bullet \to N_T + 1} \left( \deflate^q(T)^{(N_T)} \right) \Big](\x)=  N_T.
  \] 
  Thus, the two sides of Equation~(\ref{eq:deflation_commutation}) agree at such a box $\x \notin \lambda 
  \left(T^{(q)} \right)$.

Now, consider $\x \in \lambda$ with $\x \in \lambda \left(T^{(q)} \right)$. 
We have $T^{(q)}(\x) = \ell$ for some $2 \leq \ell \leq q$. Indeed, $\ell = i_k$ for some $2 \leq k \leq N_T$. Then, 
\[
\pro^q(T)(\x) = 
\Big[ \decr \circ \rep_{\bullet \to q+1}  \left( T^{(q)} \right) \Big] (\x) = i_k-1.
\]
Because, by \cite[Lemma~2.1]{DPS}, the content vector of  
$
\pro^q(T)
$
 is a cyclic rotation of the content vector of $T$, this implies that 
 \[
 [\deflate^q \circ \pro^q (T)](\x) = k-1.
 \]
 
   On the other hand, observe that since $\swap_\ell$ acts trivially on $T^{(i_{N_T})}$ for $\ell > i_{N_T}$, we have $T_{i_{N_T}} = T_q.$ Hence, $T_{i_{N_T}}(\x) = T_q(\x) = i_k$.
By Lemma~\ref{lem:gappy_promotion} with $k = N_T$, 
\[ T_{i_{N_T}} = \tinflate^q \Big( \deflate^q(T)_{N_T}, \content^q(T) \Big). \]
Thus, we must have
  \[
  [ \deflate^q(T)_{N_T}](\x) = k,
  \]
   and so 
   \[
   [\pro^{N_T} \circ  \deflate^q(T)](\x) = [ \decr \circ \rep_{\bullet \to N_T+1} (\deflate^q(T)_{N_T})](\x)  =k-1.\]
   Thus, the two sides of Equation~(\ref{eq:deflation_commutation}) also agree at such boxes $\x \in \lambda 
  \left(T^{(q)} \right)$.
\end{proof}

\begin{lemma}\label{lem:no_one}
Let $T \in \inc^q(\lambda)$ with $1 \notin \mathrm{range}(T)$. Then
\begin{equation}\label{eq:no_one}
 \deflate^q \circ \pro^q(T) = \deflate^q(T). 
\end{equation}
\end{lemma}
\begin{proof}
Since $1 \notin \mathrm{range}(T)$, $\rep_{1 \to \bullet}(T)$. Hence, for all $\ell$, $\swap_\ell$ acts trivially on $\rep_{1 \to \bullet}(T)$. Thus,
\[
\pro^q(T) = \decr \circ \rep_{\bullet \to q+1} \circ \swap_q \circ \cdots \swap_2 \circ \rep_{1 \to \bullet}(T) = \decr(T).
\] 
The lemma follows.
\end{proof}

\subsection{Computation of periods}\label{sec:period}

Define a {\bf cyclic shift} map $\shift : \{0,1\}^q \to \{0,1\}^q$ on binary vectors by 
\[ 
\shift(v)_i  = \begin{cases}
v_{i+1}, & \text{for } i \neq q;\\
v_1, & \text{for } i=q.
\end{cases}
\]

The following theorem determines the order of $K$-promotion on an increasing tableau from the order of $K$-promotion on its deflation together with the order of $\shift$ on its content vector.

\begin{theorem}
Let $T \in \inc^q(\lambda)$ and suppose $\deflate^q(T) \in \incgl^k(\lambda)$. Let $\ell$ be the cardinality of the $\shift$-orbit of $\content^q(T)$ and let $\tau$ be the cardinality of the $\pro^k$-orbit of $\deflate^q(T)$. The the $\pro^q$-orbit of $T$ has cardinality 
\[
\frac{\ell \tau}{\mathrm{gcd}(k / \ell,\tau)}.
\]
\end{theorem}
\begin{proof}
Fix $T\in \inc^q(\lambda)$ with $\deflate^q(T) \in \incgl^k(\lambda)$. We need to determine the least positive $n$ such that $(\pro^q)^{\circ n}(T) = T$.

Define $\defpro^q : \inc^q(\lambda) \to \coprod_{0 \leq k \leq q} \left( \incgl^k(\lambda) \times \binom{[q]}{k} \right)$ by
\[
\defpro^q ( T) =
\begin{cases}
    \Big( \pro^{N_T} \circ \deflate^q(T),\shift \circ \content^q(T) \Big),  & \text{if } 1 \in {\rm range}(T); \\        
   \Big( \deflate^q(T),\shift \circ \content^q(T) \Big), & \text{otherwise.}
\end{cases}
\]
Then by Lemmas~\ref{lem:deflation_commutation} and~\ref{lem:no_one}, $\compress^q \circ (\pro^q)^{\circ n}(T) = (\defpro^q)^{\circ n} \circ \compress^q(T)$. Now, since 
\[
\compress^q : \inc^q(\lambda) \hookrightarrow \coprod_{0 \leq k \leq q} \left( \incgl^k(\lambda) \times \binom{[q]}{k} \right)
\]
 is injective, $(\pro^q)^{\circ n}(T) = T$ exactly when $\compress^q \circ (\pro^q)^{\circ n}(T) = \compress^q ( T)$. Thus, our task is equivalent to determining the least positive $n$ such that 
\begin{equation}\label{eq:prostar_stabilizer}
 (\defpro^q)^{\circ n} \circ \compress^q (T) = \compress^q (T).
\end{equation}
 
Clearly, if Equation~(\ref{eq:prostar_stabilizer}) holds, then we must have 
\[
\shift^{\circ n}(\content^q(T)) = \content^q(T).\]  In addition, we must have that 
\[
(\pro^{N_T})^{\circ n'}(\deflate^q(T)) = \deflate^q(T),
\]
 where 
 \[
 n' = n - |\{ 1 \leq \ell \leq n : 1 \notin \mathrm{range}((\pro^q)^{\circ \ell}(T)) \}|
 \] 
  the number of times that $K^\ell(W(T))$ does not have $1$ in its content vector for $\ell \in 1,...,n$. 
Now say there are $r$ $0$'s in the first $\ell$ entries of $\content(T)$ which then repeat $q/\ell$ times. Then if $n = \ell$, $n' = \ell - r$. Now say we know that the period of $\deflate(T)$ is $\tau$. This reasoning shows that $K^n (\compress(T)) = \compress(T)$ if and only if $n = e \ell$ for some positive integer $e$ and $n = s \tau + e  r$. Therefore the period of $T$ is equal to the minimum integer that can be written of the form $e \ell$ and $s \tau + e r$ for the same $e$. But $e \ell = s\tau + er$ implies that $e(\ell-r) = s\tau$. Since $\ell > r \geq 0$ the minimum of $e \ell$ is clearly achieved when $e$ is minimal, which occurs when $e(\ell-r)$ is minimal, which is the least common multiple of $\ell-r$ and $\tau$. Thus the minimal $n$ can be expressed as 
\begin{equation*}
...
\Oliver{This should be probably very brief, given all the work done above. Mostly one just wants to call all the lemmas and chain them together. Not sure if we can prove the propeller theorem in the same proof environment, or whether it wants its own separate one.}
\end{proof}
