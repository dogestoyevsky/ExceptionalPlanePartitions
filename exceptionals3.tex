...
\end{cases}.
\]
We show by induction that $\compress^q \circ (\pro^q)^{\circ n}(T) = (K^q)^{\circ n} \circ \compress^q(T)$. HAVE TO SHOW FOR CONTENT VECTOR FIRST. SHOULD IMPACT BOTH CASES. The base case $n = 0$ is immediate. Assume that the result holds for $n-1$ and let $w = \content^q((\pro^q)^{\circ (n-1)}(T))$. Let $\pi_1: \coprod_{k \leq q}\incgl^k(\lambda) \times \binom{[q]}{k} \rightarrow \coprod_{k \leq q} \incgl^k(\lambda)$ be the projection onto the first factor. By Theorem~\ref{thm:compressbijective}, we can write
\[ (\pro^q)^{\circ (n-1)}(T) = \inflate^q_{w} \circ (K^q)^{\circ (n-1)} \circ \compress^q(T).\] 
If $\content^q((\pro^q)^{\circ (n-1)}(T)) \lbrace 1 \rbrace = 1$, then
\begin{align*}
\pi_1 \circ \compress^q \circ (\pro^q)^{\circ (n)}(T)) &= \deflate^q \circ (\pro^q)^{\circ (n)}(T) \\
&= \pro^{N_T} \circ \deflate^q \circ \inflate^q_{w} \circ (K^q)^{\circ (n-1)} \circ \compress^q(T)  &\text{by Lemma~\ref{lem:deflation_commutation},}\\
&= (\pro^q)^{\circ (n)} \circ \pi_1 \circ (K^q)^{\circ (n-1)} \circ \compress^q(T) &\text{by Theorem~\ref{thm:compressbijective}} \\ 
&= \pi_1 \circ (K^q)^{\circ (n)} \circ \compress^q(T),
\end{align*}
where the last equality holds because $\content^q (\compress^q(T))\lbrace 1 \rbrace = 1$. On the other hand, if $\content^q((\pro^q)^{\circ (n-1)}(T)) \lbrace 1 \rbrace = 0$, then 
\begin{align*}
\pi_1 \circ \compress^q \circ (\pro^q)^{\circ (n)}(T)) &= \deflate^q \circ (\pro^q)^{\circ (n)}(T) \\
&= \deflate^q \circ (\pro^q)^{\circ (n-1)}(T) &\text{by Lemma~\ref{lem:no_ones}} \\
&= \pi_1  \circ (K^q)^{\circ (n-1)} \circ \compress^q(T) &\text{by inductive hypothesis} \\
&= \pi_1 \circ (K^q)^{\circ (n)} \circ \compress^q(T)
\end{align*}
