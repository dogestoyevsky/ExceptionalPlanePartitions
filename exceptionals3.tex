...
Intuitively, if an order ideal is viewed as a pile of blocks, then $\mathrm{ht}$ gives the number of blocks in a given column. In this notation, that $J(\lambda \times [m])$ is exactly the set of all plane partitions of $\mathrm{ht}$ at most $m$ over the poset $\lambda$. Next, for $I \in J(\lambda \times [m])$, define $\Omega(I) \coloneqq T,$
