...
\subsection{Computation of period}\label{sec:period} We use the results of the previous section to relate the period of $T \in \inc^q(\lambda)$ under $\pro^q$ to data about $\deflate^q(T)$. Let $\Sigma^q: \lbrace 0,1\rbrace^q \rightarrow \lbrace 0,1\rbrace^q$ be the cyclic rotation so that for $(v_1,...,v_q) \in \lbrace 0,1 \rbrace^q$, $\Sigma^q(v_1,...,v_q) = (v_2,...,v_q,v_1)$. The above shows that if we define $K^q: \coprod_{s \leq q} \incgl^q(\lambda) \times \lbrace 0,1\rbrace^q \rightarrow \coprod_{s \leq q} \incgl^q(\lambda) \times \lbrace 0,1\rbrace^q$:
\begin{dmath*}
K^q(\deflate^q(T),\content^q(T)) =
\begin{cases}
    (\pro(\deflate(T)),\Sigma^q(\content(T)),  & \text{if } \content(T)(1) = 1; \\        
   (\deflate(T),\Sigma^q(\content(T)), & \text{otherwise;}
\end{cases}
\end{dmath*}
...
Take $T \in \inc^q(\lambda)$. If the period of $\deflate^q(T)$ under $\pro^q$ is $\tau$, the period of $\content^q(T)$ under $\Sigma^q$ is $\ell$, and $\content^q(T)$ has $r \cdot (q/\ell)$ zeros, then the period of $T$ under $\pro^q$ is given by Equation~(\ref{eq:period}).
\end{theorem}
