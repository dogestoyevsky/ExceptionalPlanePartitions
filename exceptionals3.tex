...
\end{tikzpicture}
\caption{The calculation of the $K$-promotion $\pro^{13}(T)$ of the increasing tableau $T$ (on the Cayley-Moufang poset) through the action of a sequence of $K$-Bender-Knuth operators $\rho_1, \dots, \rho_{12}$.}\label{fig:promotion}
\end{figure}
...
\end{lemma} 
\begin{proof}
If $\compress^q(T)\lbrace 1 \rbrace = 0$, then $\rep_{1 \rightarrow \bullet}(T) = T$. But then $\swap_\ell(T) = T$ for all $\ell$, because there are no nontrivial short ribbons containing any one integer. Also, $\rep_{\bullet \rightarrow 1}(T) = T$ since $T$ is not labelled with any bullets. Therefore $\decr$ is the only operator in Equation~\ref{eq:kprodef2} that acts nontrivially on $T$. 
\end{proof}
...
To show that Equation~\ref{eq:k_commutes} holds in the first factor, we proceed by induction. The base case $n = 0$ is immediate. Assume that the result holds for $n-1$ and let \[ w = \content^q((\pro^q)^{\circ (n-1)}(T)) = \content^q((K^q)^{\circ (n-1)} \circ \compress^q(T)). \]  By Theorem~\ref{thm:compressbijective}, we can write
...
Let $\e_i$ be the $i$th standard basis vector in $\mathbb{Z}^{2}$ and let $\pi_i : \mathbb{Z}^{2} \to \mathbb{Z}$ be the $i$th coordinate projection map for $i = 1,2$. We say that a subset $\lambda \subset \mathbb{Z}^{2}$ is {\bf connected} if for all $\x,\x' \in \lambda$, there exist $r \in \mathbb{Z}_{\geq 0}$, $d_1,...,d_r \in \{1, 2 \}$, and signs $n_1,...,n_r \in \{ \pm 1 \}$ such that 
...
Intuitively, if an order ideal is viewed as a pile of blocks, then $\mathrm{ht}$ gives the number of blocks in a given column. In this notation, $J(\lambda \times [m])$ is exactly the set of all plane partitions of $\mathrm{ht}$ at most $m$ over the poset $\lambda$. Next, define $\Omega: J(\lambda \times [m]) \rightarrow \inc^{\rank(\lambda) + m + 1}(\lambda^{\ast})$ such that 
\[ [\Omega(I)]( \x )= \mathrm{ht}(\aaa_\lambda - \x,I) + \rank(\x)+1.\] 

\begin{proposition}\label{prop:bijection}
If $\lambda$ is a justified convex shape, then  $\Omega$ is a bijection.
\end{proposition}
\begin{proof}
Since $\lambda$ is justified and convex, $\lambda^{\star}$ has an element of rank $0$ and all maximal chains in $\lambda^\star$ have equal length (Lemma~\ref{lem:max_chain_same}). Hence the function $\Omega$ coincides with the map from \cite[Proposition~8.2]{HPPW}, and so by \cite[Proposition~8.2]{HPPW} we have that $\Omega$ is a bijection between plane partitions over $\lambda$ and increasing tableaux over $\lambda^\star$.
\end{proof}
...
\begin{itemize}
\item[(a)] $(i,j,k) \in Q$ implies that $(i,j,k') \in Q$ for $0 \leq k' \leq k$;
...
Certainly, $(i',j') \leq (i,j)$. By condition (b), $\mathrm{ht}((i,j),Q) \leq \mathrm{ht}((i',j'),Q)$. Now condition (a) implies that \[ \mathrm{ht}((i'',j''),Q) = \text{max}(\{ k'' \in \mathbb{Z}_{\geq 0} : (i'',j'',k'') \in Q \})+1. \]  Therefore there is a $k'' \geq k$ with $(i',j',k'') \in Q$. But since $k' \leq k \leq k''$, condition (a) then implies $(i',j',k') \in Q$. 
\end{proof}
