...
Now, consider $\x \in \lambda$ with $\x \in \lambda \left(T^{(q)} \right)$. We observe that since $\swap_\ell$ acts trivially on $T^{(i_{N_T})}$ for $\ell > i_{N_T}$, we have $T_{i_{N_T}} = T_q.$
By Lemma~\ref{lem:gappy_promotion} with $k = N_T$, 
\[ T_{i_{N_T}} = \tinflate^q \Big( \deflate^q(T)_{N_T}, \content^q(T) \Big) \]

Now, since $\x \in \lambda(T^{(q)})$, we have that $T^{(q)}(\x) = \ell$ for $\ell \in [2,...,q]$. This implies that $\ell = n_k$ for some $2 \leq k \leq N_T$. Then, 
\[
\decr \circ \rep_{\bullet \to q+1} \circ \swap_q \circ \cdots \circ \swap_2 (T)(\x) = n_k-1.
\]
Because the content vector shifts cyclically, this implies that 
 \[
 [\deflate \circ \decr \circ \rep_{\bullet \rightarrow q+1} \circ \swap_q \circ \cdots \circ \swap_2 (T)](\x) = k-1.
 \]
  On the other hand, 
  \[
  ...
  \]
   so 
   \[
   [\decr \circ \rep_{\bullet \rightarrow N_T + 1} \circ \swap_{N_T} \circ \cdots \circ \swap_2 \circ  \deflate(T)](\x) = k-1.\]
\end{proof}
