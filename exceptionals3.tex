...
Now, consider $\x \in \lambda$ with $\x \in \lambda \left(T^{(q)} \right)$. We observe that since $\swap_\ell$ acts trivially on $T^{(i_{N_T})}$ for $\ell > i_{N_T}$, we have $T_q = T_{i_{N_T}}.$
By Lemma~\ref{lem:gappy_promotion} with $k = N_T$, 
\[ T_{i_{N_T}} = \tinflate^q \Big( \deflate^q(T)_{N_T}, \content^q(T) \Big) \]

Now, since $\x \in \lambda(T^{(q)})$, we have $T^{(q)}(\x) = \ell$ for some $2 \leq \ell \leq q$. Indeed, $\ell = i_k$ for some $2 \leq k \leq N_T$. Then, 
\[
\pro^q(T)(\x) = 
...
$
 ...
 \]
  ...
  \]
   and so 
   \[
   [\pro^{N_T} \circ  \deflate^q(T)](\x) = k-1.\]
