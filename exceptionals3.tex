...
\[
\pro^q = \decr \circ \rep_{\bullet \rightarrow q+1} \circ \swap_q \circ \cdots \circ \swap_1 \circ \rep_{1 \rightarrow \bullet}.
\]
...
%\path (T9) edge[pil,out=7,in=90,looseness=2] node[below]{$\rho_{12}$} (T9);
\end{tikzpicture}
\caption{The calculation of the $K$-promotion $\pro^{13}(T)$ of the increasing tableau $T$ (on the Cayley poset) through the action of a sequence of $K$-Bender-Knuth operators $\rho_1, \dots, \rho_{12}$.}\label{fig:promotion_via_swaps}
 \end{figure}
