...
Define $\defpro^q : \coprod_{0 \leq k \leq q} \left( \incgl^k(\lambda) \times \binom{[q]}{k} \right) \to \coprod_{0 \leq k \leq q} \left( \incgl^k(\lambda) \times \binom{[q]}{k} \right)$ by
\[
\defpro^q ( T,v ) =
\begin{cases}
    \Big( \pro^k(T),\shift(v) \Big),  & \text{if } 1 \in {\rm range}(T); \\        
   \Big( T,\shift(v) \Big), & \text{otherwise.}
\end{cases}
\]
