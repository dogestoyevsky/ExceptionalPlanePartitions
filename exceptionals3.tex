...
\begin{theorem}\label{thm:compressbijective} The product map
\[
...
\[ \inflate^q_v(k) = \min_{n \in [q]} \left( \sum_{\ell = 1}^n v(\ell) = k \right).\] Then, $\inflate^q_{\content^q(T)} \circ \deflate^q(T) = T$. Now define the {\bf tableau inflation} map 
\[
...
\]
Say $(S,v) \in \incgl^k(\lambda) \times \binom{[q]}{k}$. It is clear that for $\compress^q \circ \tinflate^q(S,v) = v$, since $S$ surjects onto $[k]$.  Also,
\begin{align*}
