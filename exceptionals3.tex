...
Now, since $\x \in \lambda(T^{(q)})$, we have that $T^{(q)}(\x) = \ell$ for some $\ell \in [2,...,q]$. This implies that $\ell = i_k$ for some $2 \leq k \leq N_T$. Then, 
\[
...
&= \pro^{N_T} \circ \pi_1 \circ (K^q)^{\circ (n-1)} \circ \compress^q(T) &[\text{Proposition~\ref{prop:compressbijective}}] \\ 
&= \pi_1 \circ (K^q)^{\circ (n)} \circ \compress^q(T) &[\text{$w\lbrace 1 \rbrace = 1$].}
\end{align*}
On the other hand, if $w \lbrace 0 \rbrace = 0$, then 
\begin{align*}
\pi_1 \circ \compress^q \circ (\pro^q)^{\circ (n)}(T)) &= \deflate^q \circ (\pro^q)^{\circ (n)}(T) \\
&= \deflate^q \circ (\pro^q)^{\circ (n-1)}(T) &[\text{Lemma~\ref{lem:no_ones}, $w \lbrace 1 \rbrace = 0$]} \\
&= \pi_1  \circ (K^q)^{\circ (n-1)} \circ \compress^q(T)  \\
&= \pi_1 \circ (K^q)^{\circ (n)} \circ \compress^q(T) &[\text{since $w\lbrace 1 \rbrace = 0$].}
\end{align*}
