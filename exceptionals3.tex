...
Fix $T \in \inc^q(\lambda)$. Let $\tau$ be the the period of $\pro^q$ on $\deflate^q(T)$ and $\ell$ be the period of $\Sigma^q$ on $\content^q(T)$. If the number of $0$'s in $\content^q(T)$ is $\frac{rq}{\ell}$, then the period of $\pro^q$ on $T$ is given by Equation~(\ref{eq:period}).
\end{theorem} 
    
  
  The above shows that if we define 
  \[
  K^q: \coprod_{s \leq q} \incgl^q(\lambda) \times \lbrace 0,1\rbrace^q \rightarrow \coprod_{s \leq q} \incgl^q(\lambda) \times \lbrace 0,1\rbrace^q
  \] by
\[
