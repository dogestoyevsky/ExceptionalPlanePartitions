...
Let $T \in \inc^q(\lambda)$ with $1 \notin \mathrm{range}(T)$. Let $1 < i_1 < \dots < i_{N_T}$ be the ordered list of indices such that $\content^q(T)(i_\ell) = 1$. Then
...
\defpro^q ( T) =
\begin{cases}
    \Big( \pro^k \circ \deflate^q(T),\shift \circ \content^q(T) \Big),  & \text{if } 1 \in {\rm range}(T); \\        
   \Big( \deflate^q(T),\shift \circ \content^q(T) \Big), & \text{otherwise.}
\end{cases}
\]
Then, $\compress^q \circ (\pro^q)^{\circ n}(T) = (\defpro^q)^{\circ n} \circ \compress^q(T)$ \oliver{call some lemmas here!}. Now, since 
\[
