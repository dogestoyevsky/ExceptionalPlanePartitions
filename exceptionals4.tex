...
Putting these facts together, we have shown that $\lambda \left(T^{(i_j)} \right) = \lambda \left(\deflate^m(T)^{(j)} \right)$ for all $j$, and that moreover, if $\deflate^m(T)_j(\x) = h$, then $T_j(\x) = \inflate^m_{\content^m(T)}(h)$. 
\end{proof}
...
Now say $\x \in \lambda(T^{(m)})$ and let $i_1, \ldots, i_{N_T}$ be as in Lemma~\ref{lem:bullet_placement}. We observe that since $\swap_\ell$ does not affect the tableau $\swap_{N_T} \circ \cdots \circ \swap_1 \circ \rep_{1 \rightarrow \bullet} (T)$ for $\ell > i_{N_T}$, we have that
...
\[
\decr \circ \rep_{\bullet \to m+1} \circ \swap_m \circ \cdots \circ \swap_1 (T)(\x) = i_j-1.
\]
Because the content vector shifts cyclically, this implies that 
 \[
 [\deflate^m \circ \decr \circ \rep_{\bullet \rightarrow m+1} \circ \swap_m \circ \cdots \circ \swap_1 (T)](\x) = j-1.
 \]
  On the other hand, 
  \[
  [\swap_m \circ \cdots \circ \swap_1 \circ \deflate^m(T)](\x) = j,
  \]
   so 
   \[
   [ \pushQED{\qed} \decr \circ \rep_{\bullet \rightarrow N_T + 1} \circ \swap_{N_T} \circ \cdots \circ \swap_1 \circ  \deflate^m(T)](\x) = j-1. \qedhere \popQED \] \let\qed\relax
\end{proof}
...
Therefore
\[ n = \frac{\ell \tau}{\text{gcd}(N_T/d,\tau)}, \]
as desired. 
\end{proof}
...
In the notation of Theorem~\ref{thm:periodthm}, say that $\tau$ divides $j N_T$ for some $j \in \mathbb{N}$. Then the period of $\pro^m$ on $T$ divides $j m$. 
\end{corollary} 
\begin{proof}
First say $\tau = j N_T$. Then
\begin{equation}~\label{eq:pdbound} \frac{\ell \tau}{\text{gcd}(N_T/d,\tau)} = \frac{m/d \cdot j N_T}{\text{gcd}(N_T/d,j N_T)} = \frac{m/d \cdot j N_T}{N_T/d} = jm. 
\end{equation}
