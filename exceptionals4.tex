...
\[
\frac{(1 - q^{h_1 + k})(1 - q^{h_2 + k})\cdots(1 - q^{h_t + k})}{(1 - q^{h_1})(1 - q^{h_2})\cdots(1 - q^{h_t})},
\]
...
\[
\incgl(\lambda) \coloneqq \bigsqcup_{m} \incgl^m(\lambda)
\]
...
A two-sided inverse for $\compress^m$ is given as follows. Let $[j]$ denote the set $\{1, 2, \dots, j\}$.
...
\[ \inflate^m_v(k) = \min_{n \in [m]} \left( \sum_{\ell = 1}^n v\lbrace \ell \rbrace = k \right).\] (We use curly braces $``\{\}"$ throughout to denote vector components.) An integer $j \in [m]$ is in the range of $\inflate^m_v$ if and only if $v\lbrace j \rbrace = 1$. From this fact and the definitions, it follows that $\inflate^m_{\content^m(T)} \circ \deflate^m(T) = T$. Now define the {\bf tableau inflation} map 
\[
...
\]
Say $(S,v) \in \incgl^n(\lambda) \times \binom{[m]}{n}$. Since $S$ is surjective and $\tinflate^m(S,v)$ maps onto the indices of nonzero components in $v$, $\content^m \circ \tinflate^m(S,v) = v$.  Also,
\begin{align*}
...
For $T \in \inc^m(\lambda)$, we define the tableau $T^{(N)}: \lambda \rightarrow \lbrace 2, \dots, m, \bullet \rbrace$ by 
...
\end{lemma} 
\begin{proof}
If $\compress^m(T)\lbrace 1 \rbrace = 0$, then $\rep_{1 \rightarrow \bullet}(T) = T$. But then $\swap_\ell(T) = T$ for all $\ell$, since there are no nontrivial short ribbons containing the labels $\bullet$ and $\ell$. Also, $\rep_{\bullet \rightarrow 1}(T) = T$, since there is not $\bullet$ label in $T$. Therefore, $\decr$ is the only operator in Equation~(\ref{eq:kprodef2}) that acts nontrivially on $T$. 
\end{proof}
...
Let $T \in \inc^m(\lambda)$ with $\content^m(T) \lbrace 1 \rbrace = 1$. Let $i_k = \inflate^m_{\content^m(T)}(k)$ for $k = 1,...,N_T$. Then, for all $\x \in \lambda$ and $1 \leq k \leq N_T$, we have
...
$q$ & orbit size & number of orbits\\
  ...
  \hline
\end{tabular}
\caption{The distribution of $\pro^q$-orbits of gapless increasing tableaux in $\incgl^q(\lambda_p)$ for each $q$.}
\label{tab:prop}
\end{table}
...
$M \gg 0$, the period of $\pro^M$ on $\inc^M(\lambda)$ is 
\begin{itemize}
\item $M$ for $\lambda = \lambda_p$, 
\item $M$ for $\lambda = \lambda_{CM}$, and 
\item $3M$ for $\lambda = \lambda_{F}$. 
\end{itemize}
(Here `$M \gg 0$' means $M \geq 2p$ for $\lambda = \lambda_p$, $M \geq 12$ for $\lambda = \lambda_{CM}$, and $M \geq 22$ for $\lambda = \lambda_{F}$.) For $M \in \lbrace 18,19,20, 21 \rbrace$, the period of $\pro^M$ on $\inc^M(\lambda_F)$ is $M$.
\end{theorem}
\begin{proof}
...
Fix $\lambda \in \lbrace  \lambda_p, \lambda_{CM} \rbrace$, $M \gg 0$, and $d$ dividing $q$. Let $\mathcal{R}(\lambda,q,d)$ be the number of increasing tableaux $T \in \inc^q(\lambda)$ whose $\pro^q$-period divides $q/d$. Then,
...
\begin{proof}[Proof of Theorem~\ref{thm:propeller}] Fix positive integers $p$ and $q$, and suppose $d \divides q$. We have that \[ f_p(\zeta) = \frac{[q]!_q [q-(p-1)]_q}{[2p-1]!_q [q-(2p-1)]!_q [p]_q }.\]  If $d = 1$, then by Theorem~\ref{thm:mainresult}, 
\[ \mathcal{R}(\lambda_p,q,d) = f_{q,2p-1}(1) + 2 \, f_{q,2p}(1) = \binom{q}{2p-1} + 2 \binom{q}{2p} = \frac{(2q-2p+1)q!}{(2p)!(q-2p+1)!} = f_p(1). \]
...
\[ \mathcal{R}(\lambda_p,q,d) = 2 \, f_{q,2p}(\zeta^{q/d}) = 2\binom{q/d}{2p/d} = f_p(\zeta^{q/d}).\]

If $d$ divides $2p-1$,  then
\[ \mathcal{R}(\lambda_p,q,d) =  f_{q,2p-1}(\zeta^{q/d}) = \binom{q/d}{(2p-1)/d} = f_p(\zeta^{q/d}).\]
If $d$ divides neither $p$ nor $2p-1$, then on the one hand Theorem~\ref{thm:mainresult} predicts that $\mathcal{R}(\lambda_p,q,d) = 0$, and on the other hand $d$ divides $\lceil{\frac{2p}{d}}\rceil$ of the terms in the numerator of $f_p(\zeta^{q/d})$ and $\lfloor{\frac{2p}{d}}\rfloor$ of the terms in the denominator, so $f_p(\zeta^{q/d}) = 0$. 
\end{proof}
...
Now let $\zeta$ be a primitive $(3M)$th root of unity. By the above, Conjecture~\ref{conj:rush.shi} claims that $f_{\lambda_F}(\zeta) = 0$. But this is impossible, for if $f_{\lambda_F}(\zeta) = 0$, then the cyclotomic polynomial $\Phi_{3M}$ would divide the numerator of $f_{\lambda_F}$, which is the product of polynomials of the form $1-q^m$ where $m < 3M$. Thus, Conjecture~\ref{conj:rush.shi} fails in these cases. 
\end{proof}
