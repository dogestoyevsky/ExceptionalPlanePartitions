...
\end{theorem}
\begin{proof}
That Conjecture~\ref{conj:rush.shi} holds for $P_F$ in the case $k \leq 4$ is a calculation using Table~\ref{tab:E7}, Theorem~\ref{thm:periodthm}, and Theorem~\ref{thm:actualpdbound}, as demonstrated in the cases $\lambda = P_p$ and $\lambda = P_{CM}$.  I SHOULD ACTUALLY CHECK THIS.
...
Now, let $\zeta$ be a primitive $(3m)$th root of unity. By the above, Conjecture~\ref{conj:rush.shi} claims that $f_{P_F}(\zeta) = 0$. But this is impossible, for if $f_{P_F}(\zeta) = 0$, then the cyclotomic polynomial $\Phi_{3m}$ would divide the numerator of $f_{P_F}$, which is the product of polynomials of the form $1-q^n$ where $n < 3m$. Thus, Conjecture~\ref{conj:rush.shi} fails in these cases. 
\end{proof}
...
%
%If $i$ is as in Equation~(\ref{eq:i}), then this row yields $i\tau N$ tableaux. In the notation of Theorem~\ref{thm:periodthm}, we have $M = q$ and  $q = m_t$. FIX THIS. Therefore, the period associated to each of these tableaux is given by Equation~(\ref{eq:p}), as required. This proves the first assertion.
%
