...
Fix $T \in \inc^q(\lambda)$. Let $\tau$ be the the period of $\pro^{N_T}$ on $\deflate^q(T)$ and $\ell$ be the period of $\Sigma^q$ on $\content^q(T)$. If $q-N_T$ (the number of $0$'s in $\content^q(T)$) is $r \cdot \frac{q}{\ell}$, then the period of $\pro^q$ on $T$ is \[\frac{\ell \tau}{\mathrm{gcd}(\ell-r,\tau)}.\]
\end{theorem} 
...
\end{table}
 ...
 In addition, the number of elements of $\binom{[M]}{k}$ whose order under $\Sigma^M$ divides a given divisor of $M$ is given by \cite[Theorem~1.1(b)]{Reiner.Stanton.White}. Therefore, we can use Tables~\ref{tab:E6},~\ref{tab:E7}, and~\ref{tab:prop} to completely determine the multiset of $\pro^M$-orbit cardinalities on any set $\inc^M(\lambda)$ for $\lambda \in \{ \lambda_{CM}, \lambda_F, \lambda_k \}$.
...
We will need the following algorithm.

\begin{algorithm}\label{alg:2}
\hspace{1cm} \phantom{2} \\
\textsf{Input}: $q, M \in \mathbb{Z}_{>0}$ with $q \leq M$. \\ 
\textsf{Output}: Array $V$. If $d_1,...,d_k$ is the decreasing list of divisors of both $q$ and $M$, then $V[\ell]$ is the number of vectors of length $M$ with $q$ ones and with period exactly $M/d_\ell$ for $\ell = 1,...,k$. \\ \-\ \\
\textsf{Initialization}: Create a list $W$ of all divisors of both $q$ and $M$ in decreasing order. Let $k = \texttt{length}(W)$. Create a vector $V$ of $k$ zeros.  \\ 
\textsf{Procedure}: For $\ell \in  \{1, 2, \ldots, k\}$:
\begin{enumerate}
\item Let $e_\ell$ be  the number of binary vectors whose $\Sigma^M$-period divides $M/d_\ell$: \[ e_\ell \coloneqq \#\left\{ v \in \binom{[M]}{q} : (\Sigma^M)^{\circ M/d_\ell}(v) = v \right\}. \]
\item Add $e_\ell$ to $V\{\ell\}$. 
\item Let $d_{\ell} \coloneqq W\{\ell\}$.
...
\begin{enumerate}
\item 
Algorithm~\ref{alg:2} computes  \[ V[\ell] = \#\left\{ v \in \binom{[M]}{q} : \text{ the $\Sigma^M$-orbit of $v$ has size exactly $M/d_\ell$ } \right\}, \ell = 1,...,k, \]
where $d_1,...,d_k$ is the decreasing list of divisors of both $q$ and $M$. 
\item All elements of $\binom{[M]}{q}$ have period $M/d_\ell$ for $\ell \in \lbrace 1,...,k \rbrace$.
\item If $\gcd(M,q) = \gcd(M',q)$, then the output of Algorithm~\ref{alg:2} is the same for $M$ and $M'$. 
\end{enumerate}
\end{theorem}
\begin{proof}
We prove (2) first. Take $v \in \binom{[M]}{q}$ and let $\pi$ be its period under $\Sigma^M$. Since rotation by $(\Sigma^M)^{\circ M}$ fixes any element of $\binom{[M]}{q}$, $\pi$ divides $M$. On the other hand, if $v\lbrace k \rbrace= 1$ then $v \lbrace k + n \pi \rbrace = 1$ for all $n \in \mathbb{Z}$, where the index is taken modulo $M$. Since $\pi$ divides $M$, it follows that the number indices $i$ such that $v \lbrace i \rbrace = 1$ is a multiple of $\pi$. 

Now for a given $d$, the known generating function for straight tableaux \cite[Theorem~1.1(b)]{Reiner.Stanton.White} tells us the number of content vectors of length $M$ with $q$ ones whose period is a multiple of $M/d$. We can then recover the number of tableaux with period exactly $M/d$ by subtracting off those whose periods strictly divide $M/d$. By the previous paragraph, it is sufficient to loop over divisors of both $q$ and $M$. This proves (1.)

Finally, say $\gcd(M,q) = \gcd(M',q)$.  Then the vector $W$ in the initialization step of Algorithm~\ref{alg:2} is the same for $M,M'$, so all steps in the algorithm are identical, except possibly the value of $e_\ell$. DISCUSSION OF METHOD USED TO COMPUTE $e_\ell$. This proves (3).
\end{proof}
...
\textsf{Output}: Array $V$, where \[ V \lbrace L \rbrace = \# \{T \in \inc^M(\lambda) : (\pro^M)^{\circ L}(T) = T \}. \] \\  \-\ \\ 
...
\end{equation}
the period associated with a tableaux of period $\tau$, a content vector of period $M/d$ with $q$ ones. 
...
In the output of Algorithm~\ref{alg:1}, $V\lbrace L \rbrace$ is equal to the number of order ideals in $J(\lambda \times \mathbf{m})$ whose order under $\Psi$ divides $L$, where $m = M -  \rank(\lambda) - 1$. 
\end{theorem}
\begin{proof}
By Corollary~\ref{cor:multisets}, it suffices to show that in the output of Algorithm~\ref{alg:1}, $V\lbrace L \rbrace$ is equal to the number of elements of $\inc^M(\lambda)$ whose period under $\pro^M$ divides $L$, where $M = \rank(\lambda)+m+1$. By Theorem~\ref{thm:compressbijective}, an element $T \in \inc^M(\lambda)$ corresponds to a unique pair $(S,v) \in \coprod_{k \leq M} (\incgl^k(\lambda) \times \binom{[M]}{k})$. Therefore, we we may instead count pairs $(S,v)$ such that the period of the inflated tableau associated to $(S,v)$ divides $L$. Now, select a row $(q,\tau,N)$ from Table~\ref{tab:E6}, Table~\ref{tab:E7}, or Table~\ref{tab:prop}. These data correspond uniquely to $\tau N$ full-content tableaux in $\incgl^q(\lambda)$. Conversely, by Theorem~\ref{thm:compressbijective}, each full-content tableaux $S \in \coprod_{k \leq M} \incgl^k(\lambda)$ must come from a row in this table. 

If $i$ is given by Equation~(\ref{eq:i}), then this row yields $i\tau N$ tableaux. In the notation of Theorem~\ref{thm:periodthm}, we have $\ell = M/d$, $\tau = \tau$, and \[ \ell - r = \frac{M}{d} - \frac{M-q}{d} = \frac{q}{d}.\] Therefore, the period associated to each of these tableaux is given by Equation~(\ref{eq:p}), as required. 
\end{proof}
We have assumed in Algorithm~\ref{alg:1} that we can execute Step~1. This step is performed by the following algorithm. 

One last observation completes our proof. Part (3) of Theorem~\ref{thm:alg2} states that Equation~\ref{eq:i} is only nonzero when $d$ divides $q$ and depends only on  $\gcd{(M,q)}$. For fixed $\lambda$, the set of such $d$ is determined by the set of common divisors of $M$ and possible full-content heights of tableau on $\lambda$ (Tables~\ref{tab:E6}, \ref{tab:E7}, and \ref{tab:prop}). For instance, if $\lambda = \lambda_{CM}$, then the set of such $d$ is determined by the common divisors of $M$ and integers from 11 to 16 (Table~\ref{tab:E6}). Therefore, if $\gcd(M,q) = \gcd(M',q)$ for all such $q$, all steps of Algorithm~\ref{alg:1} will be identical, except the computation of $p$ (Equation~\ref{eq:p}). Therefore, if we define $M$ as a symbolic variable and specify $\gcd(M,q)$ for all $q$, the output of Algorithm~\ref{alg:1} is valid for all such $M$. Since there are only finitely many possible combinations of $\gcd(M,q)$, this allows us to check the predictions of the NAME THIS for arbitrary $M$. 

To summarize, we outline the code in \texttt{symbolic_Cayley-Moufang_plane.py}. This description gives the sense of our algorithm and does not correspond to the exact details of our implementation, which is available on OP's website. We fix a tableaux $\lambda$ and define a symbolic integer variable \texttt{m}. The predicted number of tableaux in $\inc^q(\lambda)$ will be a function of \texttt{m} which is interpreted as $\texttt{m} = q-(\text{rk}(\lambda)+1)$. Let $B(\lambda)$ be the number of boxes in $\lambda$. 
\-\ \\ \-\ \\ 
\noindent {\bf Algorithm 3}: \texttt{symbolic_Cayley-Moufang_plane.py} \\ \noindent
\textsf{Input}: Convex justified shape $\lambda \in \{ \lambda_{CM}, \lambda_F, \lambda_k \}$, and the corresponding input table (Table~\ref{tab:E6},~\ref{tab:E7}, or~\ref{tab:prop}).  \\ \noindent
\textsf{Output}: Array \texttt{true}. If $C_1,...,C_k$ is a list of all multisets of prime numbers which can divide a number in the first column of the input table, then
\begin{itemize}
...
\textsf{Procedure}: 
Loop over all multisets $C_i$ of prime numbers that can divide a number in the first column of the input table. 
\begin{enumerate} 
...
\item Loop over the elements of \texttt{dVals} and compute the predicted period by the generating function. Save as \texttt{GFPeriods[\texttt{m}/d]}.
