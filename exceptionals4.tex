...
\item If $\gcd(M,q) = \gcd(M',q)$, then the output of Algorithm~\ref{alg:2} is the same for $(M,q)$ and $(M',q)$. 
\end{enumerate}
\end{theorem}
\begin{proof}
...
\medskip
\noindent
(3): Finally, say $\gcd(M,q) = \gcd(M',q)$.  Then the vector $W$ in the initialization step of Algorithm~\ref{alg:2} is the same for $M,M'$, so all steps in the algorithm are identical, except possibly the value of $e_\ell$. DISCUSSION OF METHOD USED TO COMPUTE $e_\ell$. T
\end{proof}
