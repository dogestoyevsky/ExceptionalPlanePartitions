...
\end{itemize}
(Here $M \gg 0$ means $M > 2k$ for $\lambda = \lambda_k$, $M > 16$ for $\lambda = \lambda_{CM}$, and $M > 27$ for $\lambda = \lambda_{F}$.)
\end{theorem}
\begin{proof}
Inspecting Tables~\ref{tab:E6} and \ref{tab:prop} shows that, for $\lambda_k$ and $\lambda_{CM}$, the period of each gapless tableaux (column 2) divides its height (column 1). By Corollary~\ref{corr:pdbound}, this implies that for any $M$, the $\pro^M$-period of each tableau divides $M$.
Whenever $M$ is at least the maximum $N_T$ in column 1 of the corresponding table (i.e.\ $2k$ for $\lambda_k$ and $16$ for $\lambda_{CM}$),  
it is always possible to find a tableau attaining this period $M$ by taking a gapless tableau with period $2k$ or $16$ and then inflating according to a content vector with period $M$.

Similarly, inspecting Table~\ref{tab:E7} shows that the period of all gapless tableaux of height $N_T$ divides $3N_T$ for each $N_T$. Therefore Corollary~\ref{corr:pdbound} gives that for every $M$, the period of each tableau divides $3M$. This period is attained, since any gapless tableau of height $N_T$ and period $3N_T$ may be inflated according to a content vector of period $M$ to a height $M$ tableau of period $3M$. 
\end{proof}

An analogous period calculation shows that the conjectured cyclic sieving fails for the Freudenthal poset.

\begin{proof}[Proof of Theorem~\ref{thm:exceptionals}] Let $M = 22$. Since there is a full-content tableau of height $22$ and period $66$, $\pro^{22}$ has order $66$ on $\lambda_F$. 

Theorem~\ref{thm:periodthm} and Table~\ref{tab:E7} allow us to compute that there are $0$ tableau fixed by $\pro^{22}$. For by Equation~\ref{eq:period}, the period of the content vector of such a tableau ($q/d$) would have to be $1$, so $d=22$. Since $d$ must divide the full-content height of the tableau ($N_T$), this implies that $N_T = 22$, and therefore $\gcd{(N_T/d,\tau)} = 1$, where $\tau$ is the full-content period of the tableau. But then Equation~\ref{eq:period} implies that $\tau = 1$. However, there are no full-content tableau of height $22$ and orbit size $1$. Therefore, there are no tableau fixed by $\pro^{22}$.

On the other hand, $f_{\lambda_F}(\zeta)$ cannot equal $0$ when $\zeta$ is a primitive $66$th root of unity. For this would imply that the cyclotomic polynomial $\Phi_{66}$ divides the numerator of $f_{\lambda_F}$. But this numerator is the product of functions of the form $1-x^N$ where $N < 66$. Therefore $f_{\lambda_F} \neq 0$, so cyclic sieving fails in this case. 

%However, evaluating $f_{\lambda_F}$ at a $66$th root of unity does not even yield an integer (?). 
\end{proof}
For a given $\lambda$, we denote by $N_T\{i\}$, $\tau\{i\}$, and $N\{i\}$ the $i$th element of the first, second, and third columns of the corresponding table, respectively. 
\-\ \\ \-\

\begin{theorem}~\label{thm:mainresult}
Fix $\lambda \in \lbrace  \lambda_k, \lambda_{CM} \rbrace$, $M \geq \rank(\lambda)$, and $d$ dividing $q$. Let $\mathcal{R}(\lambda,q,d)$ be the number of increasing tableaux of height $q$ whose period under $\pro^q$ divides $q/d$. Then 
\begin{equation}~\label{eq:mainresulteq} 
\mathcal{R}(\lambda,q,d)  = \sum \limits_{i: \, d \, \vert \, \frac{N_T\{i\}}{\tau\{i\}}} \tau\{i\} \, N\{i\} \, f_{q,N_T}(\zeta^{q/d}).
\end{equation}
\end{theorem}
\begin{proof}
NOTE ABOUT LOWER Q CASES
Theorem~\ref{thm:compressbijective} gives that tableaux of height $q$ correspond bijectively to pairs of full-content tableaux and content vectors of length $q$ with $N_T$ ones, where $N_T$ is the height of the full-content tableau. The table corresponding to $\lambda$ gives all possible full-content tableaux. We loop over the rows of the table and determine for each row how many content vectors yield height-$q$ tableaux whose periods divide $q/d$. 

For each $i$, let $\ell\{i\} = N_T\{i\}/\tau\{i\}$. Then $\ell[i]$ is an integer by Theorem~\ref{thm:actualpdbound}.  By Theorem~\ref{thm:periodthm}, a full-content tableau of height $N_T\{i\}$ and a content vector with period $q\{i\}/\tilde{d}$ give a tableau of period \[ \frac{\frac{q\{i\}}{\tilde{d}} \frac{N_T\{i\}}{\ell\{i\}}}{\gcd(\frac{N_T\{i\}}{\tilde{d}},\frac{N_T\{i\}}{\ell\{i\}})} = \frac{\frac{q\{i\}}{\tilde{d}} \frac{N_T\{i\}}{\ell\{i\}}}{\frac{N_T \, \gcd(\ell\{i\},\tilde{d})}{\tilde{d} \ell\{i\}}} = \frac{q\{i\}}{\gcd(\ell\{i\},\tilde{d})}. \] 
...
In the case $\lambda = \lambda_k$, the resulting formula is simple enough to check by hand. An elementary identity \cite{Reiner.Stanton.White} allows us to work with integers rather than $q$-factorial functions. Let $\zeta$ be a primitive $N$th root of unity. Then $[n]_{\zeta^{N/d}} = 0$ if and only if $n \equiv 0 \mod d$, and
...
\end{equation}
\-\ \\ \-\ 
\begin{proof}[Proof of Theorem~\ref{thm:propeller}] Fix $k$, $q$, and $d$ dividing $q$. We have that \[ f_k(\zeta) = \frac{[q]!_q [q-(k-1)]_q}{[2k-1]!_q [q-(2k-1)]!_q [k]_q }.\]  If $d = 1$, then by Equation~\ref{eq:mainresulteq}, 
\[ \mathcal{R}(\lambda_k,q,d) = f_{q,2k-1}(1) + 2 \, f_{q,2k}(1) = \binom{q}{2k-1} + 2 \binom{q}{2k} = \frac{(2q-2k+1)q!}{(2k)!(q-2k+1)!} = f_k(1). \]
Now for $d > 1$, we can have that $d$ divides $k$, or $d$ divides $2k-1$, or $d$ divides neither. If $d$ divides $k$, write $2k = \ell d$, and then 
\[ \mathcal{R}(\lambda_k,q,d) = 2 \, f_{q,2k}(\zeta^{q/d}) = 2\binom{q/d}{\ell} = f_k(\zeta^{q/d}).\]
If $d$ divides $2k-1$, write $2k-1 = \ell d$, and then
\[ \mathcal{R}(\lambda_k,q,d) =  f_{q,2k-1}(\zeta^{q/d}) = \binom{q/d}{\ell} = f_k(\zeta^{q/d}).\]
If $d$ does not divide either $k$ or $2k-1$, then on the one hand Equation~\ref{eq:mainresulteq} predicts that $\mathcal{R}(\lambda_k,q,d) = 0$, and on the other hand $d$ divides $\lceil{\frac{2k}{d}}\rceil$ of the terms in the numerator of $f_k(\zeta^{q/d})$ and $\lfloor{\frac{2k}{d}}\rfloor$ of the terms in the denominator, so $f_k(\zeta^{q/d}) = 0$. 
\end{proof}
...
\end{align*}
\end{itemize}
This exhausts all possible $d$ dividing one of the above integers. To see that the cyclic sieving polynomial predicts all other $d$ values are $0$, we first observe that $f_{CM}^q(\zeta^{N/d}) = 0$ if $d > 11$. For in this case the identity preceding Equation~\ref{eq:evalq} implies that one of the numerator terms (namely, $[q]_{\zeta^{N/d}}$) is zero, while none of the denominator terms are. 
...
Because the period associated to $\lambda_F$ may be greater than $q$, we need to modify Theorem~\ref{thm:mainresult} in this case. 
