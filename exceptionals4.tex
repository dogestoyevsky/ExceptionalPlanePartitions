...
\end{itemize}
(Here `$M \gg 0$' means $M \geq 2k +1$ for $\lambda = \lambda_k$, $M \geq 12$ for $\lambda = \lambda_{CM}$, and $M \geq 22$ for $\lambda = \lambda_{F}$.)
\end{theorem}
\begin{proof}
Inspecting Table~\ref{tab:E6} shows that, for $\lambda_{CM}$, the period of each gapless tableaux (column 2) divides its height (column 1). By Corollary~\ref{corr:pdbound}, this implies that for any $M$, the $\pro^M$-period of each tableau divides $M$.
Whenever $M$ is at least $12$,  
it is always possible to find a tableau attaining period $M$ by taking a gapless tableau with period $12$ and then inflating according to a content vector with period $M$.

Similarly, inspecting Table~\ref{tab:E7} shows that, for $\lambda_F$, the period of all gapless tableaux of height $N_T$ divides $3N_T$ for each $N_T$. Therefore, Corollary~\ref{corr:pdbound} gives that for every $M$, the period of each tableau divides $3M$. Whenever $M$ is at least $22$, the period $3M$ is attained, since any gapless tableau of height $22$ and period $66$ may be inflated according to a content vector of period $M$ to a height $M$ tableau of period $3M$. 
...
\end{theorem}
\begin{proof}
That Conjecture~\ref{conj:rush.shi} holds for $\lambda_F$ in the case $k \leq 4$ is an easy calculation using Table~\ref{tab:E7} and Theorem~\ref{thm:actualpdbound}. 

It remains to show that the conjecture fails for $k > 4$. Let $M = 22$. By Theorem~\ref{thm:actualpdbound}, $\pro^{22}$ has order $66$ on $\inc^{22}(\lambda_F)$. 

Suppose $T \in \inc^{22}(\lambda_F)$ were fixed by $\pro^{22}$.  By Equation~\ref{eq:period}, the $\Sigma^{22}$-period of $\content^{22}(T)$ is $1$. Hence $d \coloneqq \frac{M}{1} = 22$. Since $d$ must divide $N_T$, the height of $\deflate(T)$, we have also $N_T = 22$. Therefore, $T$ is gapless. However, Table~\ref{tab:E7} shows that no element of $\incgl^{22}(\lambda_F)$ is fixed by $\pro^{22}$, so such a $T$ does not exist.
...
Now let $\zeta$ be a primitive $66$th root of unity. By the above, Conjecture~\ref{conj:rush.shi} claims that $f_{\lambda_F}(\zeta) = 0$. But this is impossible, for otherwise the cyclotomic polynomial $\Phi_{66}$ would divide the numerator of $f_{\lambda_F}$, which is the product of functions of the form $1-x^N$ where $N < 66$. Thus, cyclic sieving fails in these cases. 
\end{proof}
