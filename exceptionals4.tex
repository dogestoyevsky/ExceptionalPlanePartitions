...
\[ \inflate^m_v(k) = \min_{n \in [m]} \left( \sum_{\ell = 1}^n v\lbrace \ell \rbrace = k \right).\] (We use curly braces $``\{\}"$ throughout the paper to denote vector components.) An integer $j \in [m]$ is in the range of $\inflate^m_v$ if and only if $v\lbrace j \rbrace = 1$. From this fact and the definitions, it follows that $\inflate^m_{\content^m(T)} \circ \deflate^m(T) = T$. Now define the {\bf tableau inflation} map 
\[
