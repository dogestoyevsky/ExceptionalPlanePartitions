...
\[ f_{\lambda_p}(\zeta^{m/d}) = \frac{(m-(2p-d)) ...  (m)}{(d) ...  (2p-d) p} = \frac{2p}{p}  \binom{m/d}{2p/d} = 2 \binom{m/d}{2p/d}\]    
as desired. 
...
\begin{proof}[Proof of Theorem~\ref{thm:propeller}] Fix positive integers $p$ and $m$, and suppose $d \divides m$. We have that \[ f_p(\zeta) = \frac{[q]!_q [q-(p-1)]_q}{[2p-1]!_q [q-(2p-1)]!_q [p]_q }.\]  If $d = 1$, then by Theorem~\ref{thm:mainresult}, 
