...
Fix $T \in \inc^m(\lambda)$. Let $\tau$ be the the period of $\pro^{N_T}$ on $\deflate^m(T)$ and $\ell$ be the period of $\Sigma^m$ on $\content^m(T)$. If $d = \frac{m}{\ell}$, then the period of $\pro^m$ on $T$ is \[\frac{m/d \cdot \tau}{\mathrm{gcd}(N_T/d,\tau)}. \]
\end{theorem} 
...
\begin{equation}~\label{eq:pdbound} \frac{m/d \cdot \tau}{\text{gcd}(N_T/d,\tau)} = \frac{m/d \cdot j N_T}{\text{gcd}(N_T/d,j N_T)} = \frac{m/d \cdot j N_T}{N_T/d} = jm. 
\end{equation}
