...
We use these intermediate products to characterize the action of $K$-promotion on gappy tableaux. There are two cases based on the content of the tableau. 
...
\end{equation}
\end{lemma} This result is intuitive, since after the ``initialization'' that replaces $1$s with $\bullet$s, the steps of $K$-promotion depend essentially on the relative magnitude of the labels. A careful proof requires the following technical arguments (Lemmas~\ref{lem:bullet_placement} and ~\ref{lem:gappy_promotion}). 
