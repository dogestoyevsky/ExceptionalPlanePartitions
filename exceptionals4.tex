...
\textsf{Output}: Vector $V$. If $d_1 > \dots > d_k$ is the decreasing list of all positive common divisors of $q$ and $M$, then for $\ell \in \{1, \ldots, k\}$, $V\{\ell\}$ records the number of $v \in \binom{[M]}{q}$ with $\Sigma^M$-period exactly $M/d_\ell$. \\ \-\ \\
\textsf{Initialization}: Let $d_1 > \dots > d_k$ be the decreasing list of all positive common divisors of $q$ and $M$. Create a vector $V$ of $k$ zeros.  \\ 
...
The vector $V$ computed by Algorithm~\ref{alg:2} satisfies \[ V\{\ell\} = \#\left\{ v \in \binom{[M]}{q} : \text{ the $\Sigma^M$-orbit of $v$ has size exactly $M/d_\ell$ } \right\},  \]
for all $\ell \in \{1,\ldots,k \},$
where $d_1> \dots > d_k$ is the list of all positive common divisors of $q$ and $M$.  All elements of $\binom{[M]}{q}$ have period $M/d_\ell$ for some $\ell \in \lbrace 1,\ldots,k \rbrace$.
\end{theorem}
\begin{proof}
Take $v \in \binom{[M]}{q}$ and let $\pi$ be its period under $\Sigma^M$. Since $(\Sigma^M)^{\circ M}$ is the identity on $\binom{[M]}{q}$, $\pi$ divides $M$. On the other hand, if $v\lbrace k \rbrace= 1$ then $v \lbrace k + n \pi \rbrace = 1$ for all $n \in \mathbb{Z}$, where the index is taken modulo $M$. Since $\pi$ divides $M$, it follows that the number of indices $i$ such that $v \lbrace i \rbrace = 1$ (that is, $q$) is a multiple of $\pi$. 
