...
\end{itemize}
(Here `$M \gg 0$' means $M \geq 2p$ for $\lambda = \lambda_p$, $M \geq 12$ for $\lambda = \lambda_{CM}$, and $M \geq 22$ for $\lambda = \lambda_{F}$.) For $M \in \lbrace 18,19,20 \rbrace$, the period of $\pro^M$ on $\inc^M(\lambda_F)$ is $M$.
\end{theorem}
\begin{proof}
