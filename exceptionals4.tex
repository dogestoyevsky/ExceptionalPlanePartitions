...
\end{table}
 ...
 In addition, the number of elements of $\binom{[M]}{k}$ whose order under $\Sigma^M$ divides a given divisor of $M$ is predicted by NAME THIS THING (CITE THIS FACT). Therefore, we can use Tables~\ref{tab:E6},~\ref{tab:E7}, and~\ref{tab:prop} to completely determine the multiset of $\pro^M$-orbit cardinalities on any set $\inc^M(\lambda)$ for $\lambda \in \{ \lambda_{CM}, \lambda_F, \lambda_k \}$.
