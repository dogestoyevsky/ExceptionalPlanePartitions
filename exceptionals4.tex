...
Fix $q$. By Theorem~\ref{thm:rsw}, for a given $M$ we have that FIX Q NOTATION ISSUES \[ f_{\ell} = f_{M,q}(\zeta^{M/d_{\ell}}) = \frac{[M-q+1]_{M/d_{\ell}} \cdot ... \cdot [M]_{M/d_{\ell}}}{[1]_{M/d_{\ell}} \cdot ... \cdot [q]_{M/d_{\ell}}}.\]
We can convert this to a polynomial function by applying the following elementary identity (CITE):
...
\end{cases}
\end{equation}
Now by the assumption on $M$ and $M'$, $M \equiv M' \mod d_i$ for $i = 1,...,k$.  
