...
\hspace{1cm} \phantom{2} \\
\textsf{Input}: Positive integer $L \in \mathbb{Z}_{>0}$,  list $W$ of all divisors of both $L$ and $f$ (in decreasing order). \\ 
...
\begin{enumerate}
\item Let $e_\ell \coloneqq \#\left\{ v \in \binom{[L]}{f} : (\Sigma^L)^{\circ \ell}(v) = v \right\}$, the number of binary vectors whose $\Sigma^L$-period divides $\ell$.
\item Add $e_\ell$ to $V\{\ell\}$. 
\item Let $d_{\ell} \coloneqq W\{\ell\}$.
\item For $m \in \{1,2, \dots,\ell-1\}$:
\begin{enumerate}
\item if $d_m$ divides $d_\ell$, then subtract $V\{m\}$ from  $V\{\ell\}$.
\end{enumerate} 
\end{enumerate} \-\
\end{algorithm}
\begin{theorem}~\label{thm:alg2}
Algorithm~\ref{alg:2} computes the number of vectors of length $T$ with $f$ ones and period exactly $T/d_{\ell}$ CHANGE THESE VARIABLEs.
\end{theorem}
\begin{proof}
Only a subset of divisors of $L$ will yield a nonzero number of content vectors. Since the number of ones in such a content vector is equal to $q$, the full-content height of the tableau, if the content vector divides into $d$ identical segments, then $L-q$ must be divisible by $d$. Therefore, $d$ must be a divisor of both $L$ and $q$. While $L$ is arbitrary, $q$ takes only finitely many values. 

Now for a given $d$, the known generating function for straight tableaux \cite[Theorem~1.1(b)]{Reiner.Stanton.White} tells us the number of content vectors of length $L$ with $q$ ones whose period is a multiple of $L/d$. Since this number will be zero for all but finitely many integers $d$, we can then recover the number of tableaux with period exactly $L/d$ by subtracting off those whose periods strictly divide $L/d$. 
\end{proof}
