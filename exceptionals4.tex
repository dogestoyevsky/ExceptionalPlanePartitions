...
For a given $\lambda$, we denote by $N_T[i]$, $\tau[i]$, and $q[i]$ the $i$th element of the first, second, and third columns of the corresponding table, respectively. 
\-\ \\ \-\
\begin{theorem}~\label{thm:mainresult}
Fix $\lambda \in \lbrace \lambda_{CF}, \lambda_k, \lambda_{CM} \rbrace$, $M \geq \rank(\lambda)$, and $d$ dividing $M$. Then the number of increasing tableaux of height $q$ whose period under $\pro^q$ divides $M/d$ is 
\[ \sum \limits_{i: \, d \, \vert \, \frac{q[i]}{\tau[i]}} \tau[i] \, N[i] \, f_{M,N_T}(\zeta^{M/d}).\]
\end{theorem}
\begin{proof}
