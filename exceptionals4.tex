...
\item If $p \divides L$ and $L \divides M$, add $i \tau N$ to $V\lbrace L \rbrace$.
\end{enumerate}
\end{itemize}
\end{algorithm}
\-\ 
...
If $i$ is as in Equation~(\ref{eq:i}), then this row yields $i\tau N$ tableaux. In the notation of Theorem~\ref{thm:periodthm}, we have $\ell = M/d$, $\tau = \tau$, and \[ \ell - r = \frac{M}{d} - \frac{M-q}{d} = \frac{q}{d}.\] Therefore, the period associated to each of these tableaux is given by Equation~(\ref{eq:p}), as required. 
\end{proof}
