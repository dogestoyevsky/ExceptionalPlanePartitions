...
\[\#\left\{ v \in \binom{[M]}{q} : (\Sigma^M)^{\circ k}(v) = v \right\} = f_{M,k}(\zeta^k),\] where $\zeta$ is any primitive $M$th root of unity. \qed
\end{theorem} 
We will use Theorems~\ref{thm:periodthm} and~\ref{thm:rsw}, together with the data of Tables~\ref{tab:E6},~\ref{tab:E7}, and~\ref{tab:prop}, to completely determine the multiset of $\pro^M$-orbit cardinalities on any set $\inc^M(\lambda)$ for $\lambda \in \{ \lambda_{CM}, \lambda_F, \lambda_k \}$. We will first need the following elementary algorithm.
