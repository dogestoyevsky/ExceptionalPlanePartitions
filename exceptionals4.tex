...
Fix $T \in \inc^m(\lambda)$. Let $\tau$ be the the period of $\pro^{N_T}$ on $\deflate^m(T)$ and $\ell$ be the period of $\Sigma^m$ on $\content^m(T)$. If $d = \frac{m}{\ell}$, then the period of $\pro^m$ on $T$ is \[\frac{\ell  \tau}{\mathrm{gcd}(N_T/d,\tau)}. \]
\end{theorem} 
