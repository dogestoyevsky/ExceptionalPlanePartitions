...
\textsf{Input}:  
Convex justified shape $\lambda \in \{ \lambda_{CM}, \lambda_F, \lambda_k \}$, $L \in \mathbb{Z}_{>0}$, and the corresponding Table~\ref{tab:E6},~\ref{tab:E7}, or~\ref{tab:prop}. \\
\textsf{Output}: $x = \# \{T \in \inc^q(\lambda) : (\pro^q)^{\circ L}(T) = T \} $, the number of tableaux whose period divides $L$. \\  \-\ \\ 
...
\item Compute 
\[ p \coloneqq \frac{L/d \cdot \tau}{\mathrm{gcd}(L/d-r,\tau)},\]
the period associated with $T$ and a content vector of period $L/d$, as in Theorem~\ref{thm:periodthm}. WHAT IS $r$, I'M SO CONFUSED. CAN WE DO EVERYTHING WITH $d$ INSTEAD OF $L/d$?
...
The output of Algorithm~\ref{alg:1} is equal to the number of order ideals in $J(\lambda \times \mathbf{m})$ whose order under $\Psi$ divides $L$. 
\end{theorem}
\begin{proof}
