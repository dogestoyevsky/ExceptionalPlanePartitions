...
>>>>>>> 73817c29c8fcf46fc87367ae30d93836c8f17b60
...
\end{itemize}
<<<<<<< HEAD
(Here `$M \gg 0$' means $M \geq 2k$ for $\lambda = \lambda_k$, $M \geq 12$ for $\lambda = \lambda_{CM}$, and $M \geq 22$ for $\lambda = \lambda_{F}$.) For $M \in \lbrace 18,19,20 \rbrace$, the period of $\pro^M$ on $inc^M(\lambda_F)$ is $M$.
||||||| merged common ancestors
...
\end{theorem}
\begin{proof}
That Conjecture~\ref{conj:rush.shi} holds for $\lambda_F$ in the case $k \leq 4$ is a calculation using Table~\ref{tab:E7}, Theorem~\ref{thm:periodthm}, and Theorem~\ref{thm:actualpdbound}, as demonstrated in the cases $\lambda = \lambda_k$ and $\lambda = \lambda_{CM}$.  I SHOULD ACTUALLY CHECK THIS.
