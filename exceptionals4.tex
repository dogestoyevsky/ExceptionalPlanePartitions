...
In this subsection, we recall results of \cite{Dilks.Striker.Vorland} (building on \cite{DPS}) that enable us to study the rowmotion action $\Psi$ via the action of $K$-promotion on increasing tableaux. (The framework in \cite{Dilks.Striker.Vorland} is much more general than we need here.)
...
\begin{proposition}[{\cite[Corollary~5.2]{Dilks.Striker.Vorland}}]
Let $P \in \{ P_{CM}, P_F, P_p \}$ and $m \in \mathbb{Z}_{\geq 0}$.
There is an equivariant bijection between $\inc^{m+\rank(P)}(P)$ under $K$-promotion and order ideals in $P \times {\bf m}$ under $\Psi$.
\end{proposition}
\begin{proof}
