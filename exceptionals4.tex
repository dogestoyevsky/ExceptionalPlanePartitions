...
Therefore the tableau has period dividing $q[i]/d$ is and only if $d$ divides both $\tilde{d}$ and $\ell[i]$. But $d$ divides $\tilde{d}$ if and only if the period of the content vector divides $q[i]/d$. Now the number of content vectors of length $q[i]$ with $N_T[i]$ ones and with period dividing $q[i]/d$ is precisely $f_{q,N_T}(\zeta^{q/d})$ (Theorem~\ref{thm:rsw}). 
\end{proof}

In the case $\lambda = \lambda_k$, the resulting formula is simple enough to check by hand. 
\-\ \\ \-\ 
\begin{proof} (Theorem~\ref{thm:propeller}) Fix $k$, $q$, and $d$ dividing $q$. If $d = 1$, then by Equation~\ref{eq:mainresulteq}, 
\[ \mathcal{R}(\lambda_k,q,d) = f_{q,2k}(1) + 2 \, f_{q,2k-1}(1) = \binom{q}{2k} + 2 \binom{q}{2k-1} = ? \]
\end{proof} 
