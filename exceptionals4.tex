...
\begin{enumerate}
\item Let $f_\ell$ be  the number of binary vectors whose $\Sigma^M$-period divides $M/d_\ell$: \[ f_\ell \coloneqq \#\left\{ v \in \binom{[M]}{q} : (\Sigma^M)^{\circ M/d_\ell}(v) = v \right\}. \]
...
\medskip
\noindent
(3): Say $\gcd(M,q) = \gcd(M',q)$.  Then, the list $d_1 > \cdots > d_k$ of divisors in the initialization step of Algorithm~\ref{alg:2} is the same for $(M',q)$ as for $(M,q)$. Thus, all steps in the algorithm are identical, except possibly the value of $f_\ell$. DISCUSSION OF METHOD USED TO COMPUTE $f_\ell$. 
\end{proof}
